...
struct LineChip: View {
    ...
    var body: some View {
        Text((text ?? line).uppercased())
            ...
            .frame(height: height)
            .background(lineColor(line, mode: mode, colors: colors, fill: true), in: RoundedRectangle(cornerRadius: 3))
    }
}
...
func figureFor(_ journey: Journey, board: BoardData?, now: Millis) -> Figure {
    ...
    if stale {
        return Figure(value: "", provenance: journey.retained == true && journey.realtime ? "Last known" : "Scheduled", past: minutes < 0)
    }
    if minutes < 0 { return Figure(value: "\(abs(minutes))", unit: "min", provenance: "Ago", past: true) }
    let liveRealtime = board?.source == "live" && journey.realtime
    if minutes == 0 { return Figure(value: "Now", provenance: liveRealtime ? "Departing" : "Scheduled") }
    if minutes > 99 { return Figure(value: "\(Int((Double(minutes) / 60).rounded()))", unit: "H", provenance: liveRealtime ? "" : "Scheduled") }
    let late = minutesBetween(journey.departure, journey.effectiveDeparture)
    return Figure(value: "\(minutes)", unit: "min", provenance: late > 0 ? "\(late) min late" : (liveRealtime ? "" : "Scheduled"))
}
...
func directionFigureFor(_ journey: Journey, now: Millis) -> Figure? {
    ...
    let change = journey.legs.dropLast().first { now < $0.effectiveArrival }
    let target = change?.effectiveArrival ?? journey.effectiveArrival
    return Figure(value: "\(max(0, minutesBetween(now, target)))", unit: "min", provenance: change == nil ? "To go" : "To change")
}
...
struct JourneyAxis: View {
    ...
    var body: some View {
        if let first = journey.legs.first {
            JourneyAxisLayout(journey: journey, large: large, progress: progress) {
                if showCap, let platform = platformText(first.fromPlatform, mode: first.mode) {
                    let title = first.mode == "ferry" && platform == "Wharf" ? platform : "\(first.mode == "ferry" ? "Wharf" : "Platform") \(platform)"
                ...
                ForEach(Array(journey.legs.dropLast().enumerated()), id: \.offset) { index, leg in
                    ...
                    if journey.legs.count <= 2 || index == 0,
                       let platform = platformText(leg.toPlatform, mode: leg.mode) {
                        pin(leg, platform: platform).layoutValue(key: AxisItemKey.self, value: .alight(index))
                    }
                    if let platform = platformText(next.fromPlatform, mode: next.mode) {
                        pin(next, platform: platform).layoutValue(key: AxisItemKey.self, value: .board(index))
                    }
    ...
    private func pin(_ leg: Leg, platform: String) -> some View {
        LineChip(line: leg.line, mode: leg.mode, text: platform,
            ...
            .fixedSize().padding(.leading, 3).background(colors.ground)
            .zIndex(2)
    }
}
