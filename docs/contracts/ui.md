...
The board, home and detail set contains forty-five frames; Settings adds
seven frames generated by `tools/check-settings-browser.js --frames assets/comps/latest`:
...
subpages returning to Settings. The release row reads `Version <revision>`;
local development reads `Version dev`.
