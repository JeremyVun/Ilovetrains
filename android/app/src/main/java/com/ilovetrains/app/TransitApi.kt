...
class TransitApi(baseUrl: String = BuildConfig.API_BASE) {
    ...
    suspend fun departures(from: Station, to: Station, modes: Set<String>, at: Long? = null): BoardData = withContext(Dispatchers.IO) {
        ...
        val connection = URI("$baseUrl/api/v1/departures?$query").toURL().openConnection() as HttpURLConnection
